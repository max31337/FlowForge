<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-zinc-400">Manage your team's tasks and assignments</p>
        </div>
    ...
    <div class="bg-zinc-800 rounded-lg border border-zinc-700 p-4">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
            ...
            <!-- Assigned To Filter -->
            <div>
                <label class="block text-sm font-medium text-zinc-300 mb-1">Assigned To</label>
        ...
        <div class="flex items-center justify-between mt-4 pt-4 border-t border-zinc-700">
            <div class="flex items-center gap-4">
                <label class="flex items-center">
                    ...
                    <span class="ml-2 text-sm text-zinc-300">Show completed tasks</span>
                </label>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-zinc-900 border-b border-zinc-700">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-zinc-300 uppercase tracking-wider">Task</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-zinc-300 uppercase tracking-wider">Status</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-zinc-300 uppercase tracking-wider">Priority</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-zinc-300 uppercase tracking-wider">Project</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-zinc-300 uppercase tracking-wider">Assigned To</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-zinc-300 uppercase tracking-wider">Due Date</th>
                ...
                <tbody class="divide-y divide-zinc-700">
                    @forelse($this->tasks as $task)
                        <tr class="hover:bg-zinc-750 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-start">
                                    <button wire:click="toggleTaskStatus({{ $task->id }})" 
                                        ...
                                        @else
                                            <svg class="w-5 h-5 text-zinc-400 hover:text-zinc-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <circle cx="12" cy="12" r="10" stroke-width="2"/>
                                            </svg>
                                        @endif
                                    </button>
                                    <div class="min-w-0 flex-1">
                                        <p class="text-white font-medium {{ $task->status === 'completed' ? 'line-through text-zinc-400' : '' }}">
                                            {{ $task->title }}
                                        </p>
                                        @if($task->description)
                                            <p class="text-zinc-400 text-sm mt-1 {{ $task->status === 'completed' ? 'line-through' : '' }}">
                                        ...
                                        @if($task->tags && count($task->tags) > 0)
                                            <div class="flex flex-wrap gap-1 mt-2">
                                                @foreach(array_slice($task->tags, 0, 3) as $tag)
                                                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs bg-zinc-700 text-zinc-300">
                                                        {{ $tag }}
                                                    </span>
                                                @endforeach
                                                @if(count($task->tags) > 3)
                                                    <span class="text-xs text-zinc-500">+{{ count($task->tags) - 3 }} more</span>
                                                @endif
                                            </div>
                                        @endif
                                    </div>
                            ...
                            <td class="px-6 py-4">
                                @if($task->project)
                                    <span class="text-white">{{ $task->project->name }}</span>
                                @else
                                    <span class="text-zinc-400">No project</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                @if($task->assignedTo)
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-3">
                                            <p class="text-white text-sm">{{ $task->assignedTo->name }}</p>
                                        </div>
                                    </div>
                                @else
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-zinc-800 rounded-lg border border-zinc-700 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <form wire:submit="updateTask">
                        <div class="bg-zinc-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <h3 class="text-lg font-medium text-white mb-4">Edit Task</h3>
